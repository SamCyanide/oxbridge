# Oxbridge project instructions

## AWS help level

- `help_level`: **HIGH** — set by Sam on 2026-09-21.
- Deliberately recorded *above* the AWS Agent Toolkit marker block so that re-running
  `aws configure agent-toolkit` (which replaces everything between the markers) does not
  erase it. Do not move it inside the markers.

## Cost guardrail

- **Default to designs that fit inside the AWS Free Tier.** Do not create resources
  that incur charges without asking for authorization first.
- Before creating or modifying AWS resources, **state explicitly which parts sit inside
  the Free Tier and which do not**, naming the relevant limit and what happens past it.
- If a requested design cannot fit the Free Tier, say so plainly and get direct
  authorization before proceeding rather than quietly building it.
- Set by Sam on 2026-09-21. Recorded above the AWS marker block so a rules refresh
  does not erase it.
